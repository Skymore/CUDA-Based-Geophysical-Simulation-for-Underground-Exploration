...
CAEx=single(zeros(nx,ny+1,nz+1));
CBEx=single(zeros(nx,ny+1,nz+1));
%UEyz=zeros(nx,ny+1,nz+1);
RAEyz=single(zeros(nx,2*(npml-1),nz-1));
RBEyz=single(zeros(nx,2*(npml-1),nz-1));
%UEzy=zeros(nx,ny+1,nz+1);
RAEzy=single(zeros(nx,ny-1,2*(npml-1)));
RBEzy=single(zeros(nx,ny-1,2*(npml-1)));

%Ey=zeros(nx+1,ny,nz+1);
CAEy=single(zeros(nx+1,ny,nz+1));
CBEy=single(zeros(nx+1,ny,nz+1));
%UEzx=zeros(nx+1,ny,nz+1);
RAEzx=single(zeros(nx-1,ny,2*(npml-1)));
RBEzx=single(zeros(nx-1,ny,2*(npml-1)));
%UExz=zeros(nx+1,ny,nz+1);
RAExz=single(zeros(2*(npml-1),ny,nz-1));
RBExz=single(zeros(2*(npml-1),ny,nz-1));

%Ez=zeros(nx+1,ny+1,nz);
CAEz=single(zeros(nx+1,ny+1,nz));
CBEz=single(zeros(nx+1,ny+1,nz));
%UExy=zeros(nx+1,ny+1,nz);
RAExy=single(zeros(2*(npml-1),ny-1,nz));
RBExy=single(zeros(2*(npml-1),ny-1,nz));
%UEyx=zeros(nx+1,ny+1,nz);
RAEyx=single(zeros(nx-1,2*(npml-1),nz));
RBEyx=single(zeros(nx-1,2*(npml-1),nz));

%Hx=zeros(nx+1,ny,nz);
CPHx=single(zeros(nx+1,ny,nz));
CQHx=single(zeros(nx+1,ny,nz));
%UHyz=zeros(nx+1,ny,nz);
RAHyz=single(zeros(nx-1,2*npml,nz));
RBHyz=single(zeros(nx-1,2*npml,nz));
%UHzy=zeros(nx+1,ny,nz);
RAHzy=single(zeros(nx-1,ny,2*npml));
RBHzy=single(zeros(nx-1,ny,2*npml));

%Hy=zeros(nx,ny+1,nz);
CPHy=single(zeros(nx,ny+1,nz));
CQHy=single(zeros(nx,ny+1,nz));
%UHzx=zeros(nx,ny+1,nz);
RAHzx=single(zeros(nx,ny-1,2*npml));
RBHzx=single(zeros(nx,ny-1,2*npml));
%UHxz=zeros(nx,ny+1,nz);
RAHxz=single(zeros(2*npml,ny-1,nz));
RBHxz=single(zeros(2*npml,ny-1,nz));

%Hz=zeros(nx,ny,nz+1);
CPHz=single(zeros(nx,ny,nz+1));
CQHz=single(zeros(nx,ny,nz+1));
%UHxy=zeros(nx,ny,nz+1);
RAHxy=single(zeros(2*npml,ny,nz-1));
RBHxy=single(zeros(2*npml,ny,nz-1));
%UHyx=zeros(nx,ny,nz+1);
RAHyx=single(zeros(nx,2*npml,nz-1));
RBHyx=single(zeros(nx,2*npml,nz-1));
